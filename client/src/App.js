...
class App extends Component {
  ...
  createPropertyBC = async (surveynumber, ownerAddress, marketValue, landType, citytown, kebele, state) => {
    const { accounts, contract } = this.state;
    await contract.methods.createProperty(surveynumber, ownerAddress, marketValue, landType, citytown, kebele, state).send({ from: accounts[0] })
  }
  ...
  login = () => {
    ...
    if (this.state.accounts[0].toLowerCase() === "0x8E84595782627AAFb031ff2f3AEc18ee5Ad4CAaA".toLowerCase()) {
      ...
      axios.get('http://localhost:5000/api/admin/' + this.state.accounts[0].toLowerCase())
        .then(res => {
          if (res.data) {
            this.setState({ who: 'admin', userinfo: { name: res.data.name, Id: res.data.Id, citytown: res.data.citytown, kebele: res.data.kebele, state: res.data.state }, code: 2 })
          } else {
